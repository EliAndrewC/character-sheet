...
def pcts(arr):
    """Return (p10, p50, p90) of ``arr`` as floats, or
...
def format_markdown(rows):
    ...
    def fmt_triple(t, signed=False):
        if t is None or t[0] is None:
            return "—"
        def f1(v):
            return f"{v:+.1f}" if signed else f"{v:.1f}"
        return " / ".join(f1(v) for v in t)
    out = [MD_INTRO]
    for reroll in (True, False):
        title = "Rerolling 10s" if reroll else "NOT rerolling 10s"
        out.append(f"\n## {title} - 10th / 50th / 90th percentile\n")
        out.append(
            "| rolled | kept | avg roll | Shosuro bonus | "
        ...
        for r in rows:
            ...
            out.append(
                f"| {r['rolled']} | {r['kept']} | "
                f"{fmt_triple(r['avg_roll'])} | "
                f"{fmt_triple(r['shosuro_bonus'])} | "
                f"{fmt_triple(r['gain'], signed=True)} | "
                f"{fmt_triple(r['sh_change'], signed=True)} | "
                f"{fmt_triple(r['net'], signed=True)} |"
            )
    return "\n".join(out) + "\n"
