...
- Eligible picks: all skills + all rollable knacks (commune and iaijutsu - iaijutsu IS eligible here, unlike for the 3rd Dan picker and the special ability; absorb void is excluded because it is never rolled) + attack + parry + wound_check + damage + initiative. Picks must be distinct.
...
- **Which school knacks are pickable (GM ruling 2026-08-26):** commune is the only Kitsune school knack eligible as a 3rd Dan pick - it is actually rolled and is not rules-excluded. Iaijutsu is excluded from the 3rd Dan (but stays eligible for the 1st and 2nd Dan pickers). Absorb void is excluded from all three pickers because it is never rolled.
- Server: `app/services/dice.py::_annotate_third_dan` (and the parallel branch in `app/services/rolls.py`) reads `Character.technique_choices.third_dan_skill_choices` (List[str]) when `applicable_to_choices_count` is set, unioning the picks into `third_dan_applicable`. Picks are skill ids or `commune`; iaijutsu and anything in `NON_ROLLABLE_KNACKS` (absorb void) are dropped defensively (the picker excludes them; this is belt-and-braces). The importer (`import_validate.py::_resolve_technique_choices`) accepts "Commune" and drops "Absorb Void" with a rules reason.
...
- Editor UI: new `kitsune-3rd-dan-picker` block in `app/templates/character/edit.html` with three dropdowns (`kitsune-3rd-dan-slot-0/1/2`), each with a "School knacks" optgroup (`KITSUNE_3RD_DAN_KNACK_IDS = ['commune']`) and a "Skills" optgroup (`KITSUNE_3RD_DAN_SKILL_IDS`, which is `MANTIS_2ND_DAN_SKILL_IDS` minus iaijutsu). Picks must be distinct (each slot disables values picked in the other two). Helper methods `kitsune3rdDanIsTakenInOtherSlot()` and `setKitsune3rdDanChoice()`.
