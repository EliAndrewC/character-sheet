...
- Editor UI: two-slot picker (`priest-1st-dan-skill` = skills only, `priest-1st-dan-combat` = attack/damage/initiative/parry/wound check) in `app/templates/character/edit.html`. Unset picks are a soft validation issue on the View Sheet (`TECHNIQUE_CHOICE_REQUIREMENTS` in `app/services/xp.py`).
