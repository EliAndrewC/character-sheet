...
async def money_add(
    ...
    # half-up to the nearest hundredth-koku (zeni) before persisting.
    # The validator must reject bool true/false (which pass
    # ``isinstance int`` in Python) so a posted ``true`` doesn't sneak
    # through as ``1.0``. Strings are rejected as malformed input.
    from app.services.status import round_to_hundredth as _round_to_hundredth
    raw_amount = body.get("amount")
    if isinstance(raw_amount, bool) or not isinstance(raw_amount, (int, float)):
        return JSONResponse(
            {"error": "amount must be a number"}, status_code=400
        )
    amount = _round_to_hundredth(raw_amount)
    if amount <= 0:
        return JSONResponse(
...
async def money_edit(
    ...
    from app.services.status import (
        ...
        compute_effective_status,
        compute_money_state,
        round_to_hundredth as _round_to_hundredth,
    )
