...
- [x] Dark Secret map: hovering an arrow lights it up (accent, thicker stroke) and shows the instant "<holder>'s dark secret is known by <knower>" tooltip; hovering a halo shows the "Righteousness is its own reward..." tooltip; leaving hides it; the group-name nav link returns to the group → `test_groups.py::test_dark_secret_map_page`
