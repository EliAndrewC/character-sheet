...
## Professions (Wave Man)

A **profession** is taken *instead of* a school - a ronin. `rules/09-professions.md` lists five (Wave Man, Worker, Merchant, Priest, Ninja) with ten abilities each and says nothing else about them; everything below is a campaign ruling from the GM, recorded in `profession-design/design.md` (the implementation checklist) and `school-features/WaveMan.md` (the rules intent). **Only the Wave Man has mechanics**; the other four are carried as data so the editor can list them greyed out, and adding one later is a data change plus its mechanics rather than a refactor.

- **A profession character has no school at all** - no School Ring, no school knacks, Dan 0, all five rings 2-5. They are the only character shape like this, which makes them the thing that trips unguarded school-derived Alpine expressions; `test_sheet_js_errors.py` sweeps a Wave Man for exactly that reason. **Foreign school knacks stay available** - with no school of their own, every knack in the game is foreign to them, so this is how a Wave Man reaches counterattack / double attack / lunge / iaijutsu.
- **Abilities are free, and rationed by total XP.** One pick at 150 XP (`starting_xp + earned_xp`, spent or not), plus one more every 15 XP. Each ability may be taken **twice** - Priest rituals once - so `Character.profession_abilities` is an **id-to-count map**, not a list of ids, and the editor uses `.btn-pip` steppers rather than checkboxes. A second copy applies the effect a second time. Ceiling for a Wave Man: 20 picks at 435 XP. `profession_ability_allowance()` in `services/xp.py` is the only place that rule lives; the editor reads the number off the debounced XP round-trip rather than re-deriving it in Alpine.
- **School and profession are mutually exclusive**, and share the editor's one `<select>` behind a `profession:` prefix. `split_school_or_profession()` in `app/services/professions.py` is the only place that prefix is understood and doubles as the allow-list (unknown or not-yet-selectable resolves to neither). Switching to a profession clears the school ring, school knacks and technique choices; **it deliberately keeps foreign knacks**.
- **Sanitize on write, validate on read.** `sanitize_profession_abilities()` drops unknown/foreign ids and clamps per-ability counts, but does NOT enforce the XP allowance - going over is a validation *error* so the player can see why, rather than picks silently vanishing. Under-spending is a soft warning that never blocks publishing.
- **Four of the ten abilities are reference text** (W2, W8, W10, and W1's second clause): they change what an *opponent* rolls, which this app does not model. They render as a "tell your GM" line with the numbers filled in for the copy count.
...
- Pure math lives in `roll_math.js` (`waveMan*`, each taking a **copy count**, not a boolean) with `tests/js/wave_man.test.js`; the formula-layer abilities (W6, W7) ride `PROFESSION_ABILITY_BONUSES` and reach the bot for free. Clicktests are `tests/e2e/test_professions.py` (mark: `professions`).
