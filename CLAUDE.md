...
- **Availability is three-state**, on `Profession.availability`: `"available"` (abilities are in the pool - Wave Man, Priest), `"preview"` (shown greyed out so players see what is coming, not takeable - Worker, Merchant), `"hidden"` (absent from the UI entirely - Ninja, whose abilities need a separate unlock feature). A boolean cannot say this, which is why the old `selectable` flag went away.
