...
**Gate on a `-k` filter naming the tests that cover your change, not on a whole mark.** A mark is only a reasonable gate when it is genuinely small. The marks are wildly uneven, and the big ones are not "targeted" in any useful sense - `rolls` alone is **782 of the suite's 1392 tests, 56% of everything**:

| mark | tests | | mark | tests |
|---|---:|---|---|---:|
| `rolls` | 782 | | `skills` | 67 |
| `school_abilities` | 136 | | `roll_history` | 45 |
...
- **Tracking state is guarded by a revision (`Character.tracking_rev`), and whole-state writers must name it.** A sheet tab holds a whole copy of the tracking state and posts all of it to `POST /characters/{id}/track`, so two writers used to overwrite each other silently (a void point spent from Discord, or in another tab, came back the next time the player clicked anything). `tracking_rev` is bumped by an ORM `before_update` listener (`models._bump_tracking_rev`) whenever a column in `TRACKING_COLUMNS` actually changes - **at the ORM layer on purpose, so every writer participates by existing**: `/track`, Night's Rest, the PCP void refresh, a party member spending a priest's conviction, the Discord commands, whatever comes next. `/track` requires `rev` in the body; a mismatch, or no `rev` at all, is `409 {"error": "stale", "tracking": {...}}` and nothing is written. The tab then `adoptServerState()`s (in `_tracking_js.html` - it rebuilds every counter and every value derived from `adventure_state`, including the dice roller's copies of the banked bonuses) and shows a notice that its last change was not applied; **the player never has to reload**. It adopts rather than retries because its "change" is a whole snapshot, and replaying that IS the bug. `save()` queues exactly one follow-up when a save is already in flight (it used to DROP it). Rules of thumb: **a new column a sheet tab holds a copy of goes in `TRACKING_COLUMNS` (with `active_history=True`, which is what lets the listener tell a real change from a re-sent value) and in `tracking_snapshot()` and `adoptServerState()`**; `money_ledger` is deliberately NOT in it (its routes are item-level, so a revision would only produce false "changed elsewhere" notices); an endpoint that bumps the revision on behalf of the tab that called it should return `tracking_rev` so that tab is not refused by its own action (`/spend-pcp` does; Night's Rest reloads). Unit tests get a current `rev` (and `build_rev`, for `/autosave`) filled in by `tests/conftest.py::_track_like_a_fresh_tab`. Tests about the handshake use `client.request("POST", ...)`.
- **The BUILD is guarded the same way, by a separate revision (`Character.build_rev`), with a different recovery.** The editor autosaves effectively the whole build, so two editor tabs (or a GM and a player who both have edit access) used to overwrite each other silently. `build_rev` is bumped by the same ORM listener (`models._bump_revisions`) whenever a column in `BUILD_COLUMNS` actually changes; `POST /autosave` requires it, and stale or missing is `409 {"error": "stale", "build_rev": N}` with nothing written. **Separate from `tracking_rev` on purpose**: a wound taken on the sheet cannot conflict with an editor tab and must not make one stale. **The editor does NOT adopt the server's state the way the sheet does** - the user may be mid-sentence - so on a 409 it stops autosaving, keeps its text and the unload warning, and asks: "Load the latest version" (reload), "Keep my version" (an ordinary save naming the revision the refusal reported - an informed overwrite, where the old one was silent), or "Decide later" (saving stays paused; a Resolve button reopens the prompt). `GET /draft-diff` names the `build_rev` it describes and `POST /discard` sends it back, so a discard cannot destroy a change that was never listed; `POST /publish` names the revision the tab last saved as. Those two check it only when one is named (they are operations on server state, not whole-object writes). Rules of thumb: **a column autosave writes goes in `BUILD_COLUMNS`** (a test reads the route's source and fails otherwise); **every build-writing request from the editor goes through `_enqueueWrite` and adopts the returned `build_rev`**, or a tab refuses itself (`/set-award-source` runs on a 600 ms timer beside the 1500 ms autosave); an endpoint that moves `build_rev` for the tab that called it returns it; the dark secret is left out of the comparison (autosave can neither read nor write it, so it cannot be lost, and the GM setting one must not throw a conflict at the player's editor). Revert (launched from the sheet) and the legacy form endpoint `POST /characters/{id}` (posted to by no template) are NOT guarded - see `discord-design/audit.md` B2/B3.
...
- **"Combat roll" is decided once, by roll key, on the server (`dice.is_combat_roll`)** and stamped on every formula as `is_combat_roll`, the same way `void_blocked` is. Attacks of every kind, parry, wound check, `athletics:attack` / `athletics:parry`, feint and iaijutsu (every variant) are combat; skills, bare ring rolls, `athletics:<Ring>` feats, initiative and every other knack are not. Mirumoto 5th Dan ("+10 per void point on combat rolls") keys on it; the amount is `void_spend.combat_vp_flat_bonus`, read by both `pages.py` and `roll_engine`. The sheet's generic roller used to add the +10 to every roll, a Sincerity roll included, because it had no such notion. **A new "on combat rolls" rule should read the flag, not grow its own list.**
...
A **profession** is taken *instead of* a school - a ronin. There is exactly one profession character type, **"Profession"**, and a profession character draws abilities from every *available* profession and mixes them freely: a wave man who settles in a city picks up a worker's habits, or is taught a ritual by the monks he prays with. `rules/09-professions.md` lists five profession ability sets with ten abilities each and says nothing else about how they work; everything below is a campaign ruling from the GM, recorded in `profession-design/design.md` and `profession-design/priest-and-pooling.md` (the implementation checklists) and `school-features/WaveMan.md` (the rules intent).

- **Availability is three-state**, on `Profession.availability`: `"available"` (abilities are in the pool - Wave Man, Priest, Worker, Merchant), `"preview"` (shown greyed out so players see what is coming, not takeable - none today), `"hidden"` (absent from the UI entirely - Ninja, whose abilities need a separate unlock feature). A boolean cannot say this, which is why the old `selectable` flag went away. **`ProfessionAbility.available` is the same gate one level down**, for an ability held back inside a live profession: the Worker's advanced-skills-as-basic is campaign-specific, so it renders greyed out rather than vanishing. Both gates must pass, which `ability_is_available()` is the single place to decide.
- **A profession character has no school at all** - no School Ring, no school knacks, Dan 0, all five rings 2-5. They are the only character shape like this, which makes them the thing that trips unguarded school-derived Alpine expressions; `test_sheet_js_errors.py` sweeps both a bare and a mixed profession character for exactly that reason. **Foreign school knacks stay available** - with no school of their own, every knack in the game is foreign to them, so this is how they reach counterattack / double attack / lunge / iaijutsu.
- **Abilities are free, and rationed by total XP.** One pick at 150 XP (`starting_xp + earned_xp`, spent or not), plus one more every 15. The allowance is **pooled** over available professions (`profession_ability_allowance()` in `services/xp.py`, the only place the rule lives), and capped at what the pool can supply so nobody is nagged about picks they cannot spend. Wave Man 20 + Priest 10 + Worker 18 + Merchant 20 = 68 picks at 1155 XP. Note a single profession runs out first: Wave Man's 20 are gone at 435 XP, so a character at 450 must branch out.
- **Most abilities may be taken twice; Priest rituals only once** - a ritual is something you were taught, not a knack you deepen. The limit follows the **ability's own profession**, so a twice-taken Wave Man ability sits beside a once-only Priest ritual in the same character. Hence `Character.profession_abilities` is an **id-to-count map**, the editor uses `.btn-pip` steppers, and Priest groups carry an "each may be taken once" chip (an unexplained disabled `+` on a page where everything else goes to 2 reads as a bug).
- **School and profession are mutually exclusive**, and share the editor's one `<select>`. The value is `profession`; `split_school_or_profession()` in `app/services/professions.py` is the only place that is understood and doubles as the allow-list. It **still accepts the legacy `profession:<id>` form** - an editor tab open across the deploy will POST it, and resolving that to "no profession" would wipe the character's abilities. Switching to a profession clears the school ring, school knacks and technique choices; **it deliberately keeps foreign knacks**.
- **Sanitize on write, validate on read.** `sanitize_profession_abilities()` drops unknown ids and anything from a preview/hidden profession, and clamps per-ability counts, but does NOT enforce the XP allowance - going over is a validation *error* so the player can see why, rather than picks silently vanishing. Under-spending is a soft warning that never blocks publishing.
- **The editor shows every visible ability; the View Sheet shows only what was taken**, both grouped by source profession. Thirty greyed-out rows would bury the handful that matter.

### Wave Man abilities

- **Four of the ten are reference text** (W2, W8, W10, and W1's second clause): they change what an *opponent* rolls, which this app does not model. They render as a "tell your GM" line with the numbers filled in for the copy count.
- **W1's attack scope is wider than `ATTACK_TYPE_KEYS`.** It reaches the iaijutsu strike, which that set deliberately excludes so a duel's strike keeps its own rules for every school. Hence the separate `is_wave_man_attack_key()` in `services/dice.py` - **do not "unify" the two**. W1 also spends only the raises it needs, and its raise earns no extra damage dice (they come off the *unraised* total).
- **W5 frees dice, it does not clear the suppression.** Impaired still stops the roll's other 10s; one die per copy is freed, **selected automatically** (every 10 is interchangeable, so there is no picker - unlike the Hida 3rd Dan flow). The freed die chains. It keys on `no_reroll_reason == "impaired"`, which is what keeps it off initiative, the duel strike, and unskilled rolls. **The Discord bot applies W5 too**: the die is auto-selected, so there is no interactive choice for a slash command to skip, and skipping it would make the same roll differ between the sheet and `/etiquette`.
- **W4's rounding runs last**, after every other damage bonus. 15 becomes 18, not 20 - a total already on a multiple of 5 rises by 3.
- **W3's armor clause is deferred**: there are no armor rules anywhere in `rules/`. The only occurrence of "armor" in the whole rules directory is inside that ability.
- Pure math lives in `roll_math.js` (`waveMan*`, each taking a **copy count**, not a boolean) with `tests/js/wave_man.test.js`; the formula-layer abilities (W6, W7) ride `PROFESSION_ABILITY_BONUSES` and reach the bot for free.

### Priest rituals

The Priest *school*'s special ability is "You have all 10 rituals", but **only three of the ten have ever had mechanics** (bless a conversation topic, bless research, and the sick-or-impaired ritual); the other seven are narrative and stay that way as profession abilities, rendered with a "tell your GM" note. See `school-features/Priest.md`.

- **The three are gated on holding the ritual, not on the Priest school.** `priest_bless_topic` and `priest_bless_research` are **separate flags** so a profession character who learned only one gets only that button; each reads `school == "priest" or holds(ritual)`.
- **The sick-or-impaired ritual may target the priest themselves** as well as an ally, for school and profession characters alike. Before this it could not: the blesser list came purely from `visible_party_members`, which excludes the viewing character. **Do not add the self entry by relaxing that function** - it is shared with Priest 5th Dan conviction, Priest 3rd Dan precepts sharing and the party-effect layer generally, and a self entry leaking into those would be a subtle bug. It is built alongside the party list in the `party_priests` block in `pages.py`.
- **The fifth ritual's "commune" shares its name with the `commune` school knack on purpose** - it is the weaker version, rolled with precepts rather than the knack, and limited to detecting influence that is present or nearby. It routes through none of the knack's machinery. Do not "fix" one into the other.

### Worker and Merchant abilities

Almost all of them are **conditional free raises**, declared in `PROFESSION_ALTERNATIVE_BONUSES` and applied in `build_skill_formula` and `build_athletics_formula`. They become "Alternative totals" rows: the condition ("relating to your business", "for feats of strength") is the player's to judge, and the label is the whole gate, exactly as the Streetwise advantage's condition works. A character with many of them will have many alternative rows; that is the intended shape. See `school-features/WorkerAndMerchant.md`.

- **An alternative total is arithmetic on a roll that already happened**, so it can add a number but never dice, and states one number per row. That constraint is why the Merchant's business-experience ability emits **two** rows (plain and contested) and why its extra-dice ability is not a row at all.
- **The Merchant's open-commerce dice are a pre-roll choice.** The commerce tile offers the plain roll and the business roll as separate rows, each with its own void submenu, the way a Ring tile offers a ring roll and its athletics variant. `PROFESSION_ROLL_VARIANTS` is generic. Slash commands stay on the plain roll - `/commerce` has nobody to ask which it is.
- **The Worker's feats of strength / endurance are the second exception** to the rule that `athletics:<Ring>` keys are only emitted for athletics-knack holders. That gate exists because the roll would otherwise duplicate the bare ring roll; a Worker bonus makes it genuinely different, same as the Kitsune Warden's 4th Dan. Without the exception the ability is unreachable.
- **The Merchant's business reroll shares the one-reroll lock** with Lucky and the PCP reroll, in both directions, and is offered on all 18 skill rolls (it is the one Merchant ability naming no skill). Its void spend goes through `deductVoidPoints`, which already early-returns for a non-editor.
- **Money bonuses accumulate into one percentage that multiplies the stipend, applied LAST** (`profession_money_bonus()` in `services/professions.py`, consumed by `compute_effective_status`). Applying it before Household Wealth would multiply the wrong base. It carries its own `stipend_modifiers` entry so the Money tooltip explains it.
...
- **`app/services/roll_engine.py` is the Python mirror of the browser's roller.** It reuses
  ...
  literal case tables in `tests/shared/`. Mirumoto 5th Dan's +10 per void point is applied by both, only where
  the formula carries `is_combat_roll` (see "Combat rolls" under Key Architectural Decisions).
